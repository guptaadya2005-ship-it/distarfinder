...
function Prediction() {
  ...
  const getRiskColor = () => {
    ...
    if (prediction.level === "Critical") return "text-red-600";
    if (prediction.level === "High") return "text-orange-500";
    if (prediction.level === "Moderate") return "text-yellow-500";
    return "text-emerald-500";
  };

  const getRiskBg = () => {
    if (!prediction) return "bg-slate-100";
    if (prediction.level === "Critical") return "bg-red-50 border-red-200";
    if (prediction.level === "High") return "bg-orange-50 border-orange-200";
    if (prediction.level === "Moderate") return "bg-yellow-50 border-yellow-200";
    ...
    <div className="min-h-screen">
      ...
      <div className="app-main">
        ...
        <main className="p-8">
          ...
          <div className="mb-8 flex items-start justify-between">
            <div>
              <div className="mb-2 flex items-center gap-2">
                <BrainCircuit size={20} className="text-[#b8862e]" />
                <span className="text-xs font-semibold uppercase tracking-widest text-[#b8862e]">
                  AI Decision Support
                </span>
              </div>
            ...
            <div className="flex items-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-2">
          ...
          {/* SELECTED ZONE — mb-6 -> mb-8 for breathing room before the grid */}
          <div className="mb-8 flex items-center justify-between glass-card p-5">
            <div className="flex items-center gap-4">
              <div className="flex h-11 w-11 items-center justify-center rounded-md bg-[#122c47]">
          ...
          {/* MAIN GRID — items-stretch forces both columns to equal height */}
          <div className="grid items-stretch gap-6 lg:grid-cols-5">
            ...
            <div className="lg:col-span-3">
              <div className="glass-card flex h-full flex-col p-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full rounded-md border border-slate-200 bg-slate-50 px-4 py-3 pr-16 text-sm font-semibold text-slate-800 outline-none transition focus:border-[#12395e] focus:bg-white focus:ring-2 focus:ring-emerald-100"
                      />
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full rounded-md border border-slate-200 bg-slate-50 px-4 py-3 pr-12 text-sm font-semibold text-slate-800 outline-none transition focus:border-[#12395e] focus:bg-white focus:ring-2 focus:ring-emerald-100"
                      />
                      <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-medium text-slate-400">
                        %
                      </span>
                ...
                {/* ANALYZE BUTTON — mt-7 -> mt-auto so it anchors to the bottom
                    of the card and the card fills its full stretched height */}
                <button
                  onClick={analyzeRisk}
                  disabled={isAnalyzing}
                  className="mt-auto flex w-full items-center justify-center gap-2 rounded-md bg-[#122c47] px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-[#122c47]/20 transition hover:bg-[#0c1f33] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {isAnalyzing ? (
                    <>
                ...
                {prediction && !isAnalyzing && (
                  <button
                    ...
                    className="mt-3 flex w-full items-center justify-center gap-2 rounded-md border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50"
                  >
                    <RotateCcw size={15} />
                    Run New Analysis
                  </button>
            ...
            <div className="lg:col-span-2">
              {!prediction && !isAnalyzing && (
                <div className="flex h-full min-h-[450px] flex-col items-center justify-center rounded-2xl border border-dashed border-white/70 bg-white/40 p-8 text-center backdrop-blur-xl">
              ...
              {isAnalyzing && (
                <div className="flex h-full min-h-[450px] flex-col items-center justify-center glass-card p-8 text-center">
                  <div className="flex h-16 w-16 items-center justify-center rounded-lg bg-emerald-50">
              ...
              {prediction && !isAnalyzing && (
                <div className={`h-full rounded-lg border p-6 shadow-sm ${getRiskBg()}`}>
                  ...
                  <div className="mt-8 text-center">
                    <p className={`text-7xl font-black tracking-tight ${getRiskColor()}`}>
                      {prediction.score}%
                    </p>
                    <div
                    ...
                    >
                      {prediction.level === "Critical" && <AlertTriangle size={15} />}
                      {prediction.level === "Critical"
                        ? "CRITICAL RISK"
                  ...
                  <div className="mt-7 rounded-md border border-white/70 bg-white/70 p-4">
                  ...
                  <button
                    ...
                    className="mt-6 flex w-full items-center justify-center gap-2 rounded-md bg-red-600 px-5 py-3.5 text-sm font-semibold text-white shadow-lg shadow-red-600/20 transition hover:bg-red-700"
                  >
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed text-slate-500">
                  TerraGuard combines rainfall, soil moisture and terrain
                  slope indicators to estimate landslide risk. In the
                  production system, this decision-support layer can be
                  connected to trained ML models, real-time sensors,
                  satellite data and historical disaster records.
                </p>
              </div>
            </div>
          </div>
